  ...
  const handleSubmitSignup = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.access_token) {
        ...
        setSuccess(true);
        displayPopup("An email has been sent to you, please check it...");

            // Fermer la popup après 3 secondes (3000 millisecondes)
            setTimeout(() => {
              hidePopup(); // ou une fonction pour la fermer
            }, 5000);
 ...
 const handleSubmitLogin = async (e) => {
    ...
    try {
      ...
      if (loginError.code) {
        ...
        // Autres erreurs
        setError("Unable to connect to the server!");
        displayPopup("Please verify your email before logging in. Check your inbox for a verification link.",  8000);
      }
    }
  };
