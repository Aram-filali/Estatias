...
const Prices: React.FC<{ 
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      try {
        // Second API call with consistent base URL
        const siteGenerationResponse = await axios.post(
          `${API_BASE_URL}/site-generator/${currentHostId}`,
          {}, // Empty body
          {
            headers: {
              Authorization: `Bearer ${idToken}`,
        ...
        if (siteGenerationResponse.data && siteGenerationResponse.data.url) {
          // Instead of showing popup, redirect to success page with the URL as a query parameter
          router.push(`/success?url=${encodeURIComponent(siteGenerationResponse.data.url)}`);
        } else {
          // If we don't get a URL back, still redirect to success page without URL
          router.push('/success');
        }
      } catch (error) {
  ...
  return (
    <div className={`${styles.pricesContainer} mt-[100px]`}>
      <section className="flex items-center justify-center min-h-screen p-4">
        <div className="w-full max-w-7xl mx-auto">
          ...
          <div className="relative top-[70px] right-[-200px] w-full flex flex-col items-center">
