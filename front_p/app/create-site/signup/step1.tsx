...
export function Step1({ nextStep, prevStep, currentStep, totalSteps }: StepProps) {
  ...
  const [selectedCountry, setSelectedCountry] = useState(countries.find(c => c.code === 'FR')); // Default to FR
  const [touched, setTouched] = useState<Record<string, boolean>>({});
  const [isClient, setIsClient] = useState(false); // Track if we're on client side
  
  // Fix: Move notification state inside the component
  ...
  }, [formData.country, isClient]);

  // Fonction de validation pour un champ spécifique
  const validateField = useCallback((name: string, value: any, formData: FormData) => {
    let error = '';
    ...
    switch (name) {
      ...
      case 'propertiesCount':
        if(value) {
          const v= value;
          if (v > 500) {
            error = 'The number of properties must not exceed 500'
          }
  ...
  const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'domainName') {
      ...
      const domainError = validateField(name, newValue, { ...formData, [name]: newValue });
      setErrors(prev => ({
        ...prev,
        [name]: domainError || ''
      }));
    ...
    const error = validateField(name, newValue, updatedFormData);
    setErrors(prev => ({
      ...prev,
      [name]: error || ''
    }));
  ...
  }, [errors, formData, phonePrefix, validateField]);
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await handleSubmit(formData, null);
        ...
        <h2 className={styles.title}>Create Your Host Account</h2>
          ...
          <div className="flex justify-between mt-4">
            ...
            <button
              ...
              className={styles.button}
            >
              {currentStep && currentStep < (totalSteps || 4) ? "Verify your email" : "Submit"}
            </button>
          </div>
        </form>
  );
